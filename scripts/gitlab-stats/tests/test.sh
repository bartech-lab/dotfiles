...
query=$(build_query org/group/repo 2026-07-01 2026-07-15 develop)
...
discussion_data=$(fetch_discussion_data org/group/repo "$base_mrs") || fail "discussion fetch failed"
...
retry_data=$(fetch_discussion_data org/group/repo '[{"iid":"102"}]') || fail "retry fetch failed"
[[ "$(<"$GLAB_RETRY_FILE")" -eq 4 ]] || fail "expected three retries before success"
unset GLAB_MODE GLAB_RETRY_FILE

if GLAB_MODE=fail-author fetch_discussion_data org/group/repo '[{"iid":"102"}]' >"$TMP_DIR/fail.out" 2>"$TMP_DIR/fail.err"; then
...
first_output=$(bash "$ROOT_DIR/gitlab-stats" --project org/group/repo --since 2026-07-01 --until 2026-07-15 --stats comments --format json --no-cache 2>"$TMP_DIR/cli-first.err") || fail "CLI comments run failed"
assert_contains "$first_output" "Reviewer | Review threads started"
before_lines=$(wc -l < "$GLAB_LOG")
all_output=$(bash "$ROOT_DIR/gitlab-stats" --project org/group/repo --since 2026-07-01 --until 2026-07-15 --stats all --format json 2>"$TMP_DIR/cli-cache.err") || fail "CLI all cached run failed"
