...
Linux only; loaded from `zsh/functions/41-ai-agents.zsh` when `systemd-run` is
present.

### claude / codex

Wrappers that run the real CLI through `ai-agent-scope`, placing the session
and every process it spawns inside the memory-capped `ai-agents.slice` cgroup.

```bash
claude              # runs inside the slice
codex exec "..."    # same

AI_AGENT_NO_SCOPE=1 claude   # bypass the cap for one command
```

### ai-agent-mem

Reports the slice's memory usage and kernel counters.
...
`high` counts reclaim-throttle events, `oom_kill` counts processes the cap
killed. Prints a notice instead when no agent is running.

### ai-agent-scope

Launcher used by the wrappers; also callable directly for any command.

```bash
ai-agent-scope <command> [args...]
ai-agent-scope --help
```

See the [AI Agent Limits README](../ai-agent-limits/README.md).
...
The wrapper uses three entry points, because one mechanism cannot cover every caller:
...
- `scripts/shims/git` symlinks to the wrapper, and `zsh/zshrc.zsh` puts
  `scripts/shims` first on the interactive shell's `PATH`.
- `install.sh` links `~/.local/bin/git` to the wrapper. Agent environments
  commonly keep `~/.local/bin` before Homebrew when they rebuild `PATH`.

The shims cover non-zsh callers. Agents such as omp, Codex and Claude Code can
run commands under bash, which never reads `~/.zshenv`.
...
- Colima is on-demand: `colima start` / `colima stop`; no login items or defaults to manage
